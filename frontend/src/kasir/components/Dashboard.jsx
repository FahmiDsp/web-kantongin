...
export default function Dashboard({
  ...
  // QRIS Settings dynamic state
  const [qrisValidation, setQrisValidation] = useState({ checked: false, valid: false, message: '' });
  const [qrCodePreview, setQrCodePreview] = useState('');
  const [qrisInputType, setQrisInputType] = useState('string'); // 'string' or 'file'

  useEffect(() => {
    if (activeModal === 'settings') {
      const currentQrisString = settings.qrisString || '';
      setSettingsForm({
        taxPercent: settings.taxPercent || 0,
        qrisString: currentQrisString,
      });
      setQrisValidation({ checked: false, valid: false, message: '' });
      setQrCodePreview('');

      if (currentQrisString) {
        setQrisInputType('string');
        const validation = isValidQRIS(currentQrisString);
        setQrisValidation({ checked: true, valid: validation.valid, message: validation.message });
        if (validation.valid) {
          QRCode.toDataURL(currentQrisString.trim(), { margin: 2, scale: 6 })
            .then((url) => setQrCodePreview(url))
            .catch((err) => console.error(err));
        }
      } else if (settings.qrisImage) {
        setQrisInputType('file');
      } else {
        setQrisInputType('string');
      }
    }
  ...
  const handleValidateQris = async () => {
    const qrisStr = settingsForm.qrisString.trim();
    const validation = isValidQRIS(qrisStr);
    setQrisValidation({ checked: true, valid: validation.valid, message: validation.message });
    if (validation.valid) {
      try {
        const qrUrl = await QRCode.toDataURL(qrisStr, { margin: 2, scale: 6 });
        setQrCodePreview(qrUrl);
      } catch (err) {
        console.error('Gagal generate preview QR Code:', err);
      }
    } else {
      setQrCodePreview('');
    }
  ...
  const handleSettingsSubmit = async (e) => {
    ...
    if (qrisInputType === 'string') {
      ...
      if (file) {
        ...
        finalQrisImage = await readFileAsDataUrl(file);
      } else {
        finalQrisImage = settings.qrisImage || '';
      }
    }

    onSaveSettings(numberOnly(settingsForm.taxPercent), finalQrisImage, finalQrisString);
    setActiveModal(null);
  };
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Kelola data barang, stok masuk, stok kadaluarsa, dan profil toko.
          </p>
        </div>
      </div>
                ...
                filteredProducts.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          {p.photo ? (
                            ...
                            <div
                              className="w-10 h-10 rounded-lg flex items-center justify-center font-bold text-white uppercase text-xs"
                              style={{ backgroundColor: p.color || '#15803d' }}
                            >
                      ...
                      <td className="px-5 py-3.5 text-right font-semibold text-primary-600">{rupiah(p.price)}</td>
                      <td className="px-5 py-3.5 text-right font-bold text-accent-600">{rupiah(profit)}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-gray-100 animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleProductSubmit} className="p-5 flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-primary-500 text-sm outline-none"
                    required
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-primary-500 text-sm outline-none text-right font-semibold"
                    required
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-primary-500 text-sm outline-none text-center font-bold"
                    required
                  />
                </div>
                <div>
              ...
              {editingProduct && editingProduct.photo && (
                <div className="flex items-center gap-4 p-2.5 border border-gray-200 rounded-xl bg-gray-50">
                  ...
                  <div>
                    ...
                    <button
                      type="button"
                      onClick={() => onRemoveProductPhoto(editingProduct.id)}
              ...
              <button
                type="submit"
                className="w-full py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl shadow-md transition-all active:scale-95 flex items-center justify-center gap-1.5"
              >
            ...
            <form onSubmit={handleStockSubmit} className="p-5 flex flex-col gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white outline-none focus:border-primary-500"
                  required
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-primary-500 text-sm text-center font-bold"
                  required
                />
              </div>
              ...
              <button
                type="submit"
                className="w-full py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl shadow-md transition-all active:scale-95"
              >
                Simpan Mutasi Stok
              </button>
            </form>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden border border-gray-100 animate-in zoom-in-95 duration-200">
            <div className="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
              <h3 className="font-bold text-red-600 flex items-center gap-1.5">
                <ShieldAlert className="w-5 h-5" /> Catat Barang Kadaluarsa
              </h3>
              <button onClick={() => setActiveModal(null)} className="text-gray-400 hover:text-gray-600">
                <X className="w-5 h-5" />
              </button>
            ...
            <form onSubmit={handleExpiredSubmit} className="p-5 flex flex-col gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Masukkan angka..."
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-primary-500 text-sm text-center font-bold text-red-600"
                  required
                />
              </div>
              ...
              <button
                type="submit"
                className="w-full py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl shadow-md transition-all active:scale-95"
              >
                Simpan & Potong Stok
              </button>
            </form>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden border border-gray-100 animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSettingsSubmit} className="p-5 flex flex-col gap-4">
              ...
              <div className="border border-gray-150 rounded-xl p-3 bg-gray-50 flex flex-col gap-3">
                <span className="block text-xs font-bold text-gray-700">Metode QRIS</span>
                <div className="flex gap-2 p-1 bg-gray-200/60 rounded-lg">
                  <button
                    type="button"
                    onClick={() => setQrisInputType('string')}
                    className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${qrisInputType === 'string' ? 'bg-white text-primary-700 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
                  >
                    Dinamis (Teks)
                  </button>
                  <button
                    type="button"
                    onClick={() => setQrisInputType('file')}
                    className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${qrisInputType === 'file' ? 'bg-white text-primary-700 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
                  >
                    Statis (Gambar)
                  </button>
                </div>

                {qrisInputType === 'string' ? (
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <label className="block text-xs font-bold text-gray-600">String QRIS Merchant</label>
                      <button
                        type="button"
                        onClick={() =>
                          alert(
                            'Cara mendapatkan String QRIS:\n1. Buka aplikasi merchant bank/e-wallet Anda, lalu cari opsi "Salin Kode QRIS" jika ada.\n2. ATAU, ambil gambar QRIS Anda, lalu scan menggunakan aplikasi QR Code Scanner / Google Lens / WhatsApp QR scanner, lalu salin teks hasil scan yang diawali dengan "000201..."'
                          )
                        }
                    ...
                    <textarea
                      value={settingsForm.qrisString}
                      onChange={(e) => {
                        setSettingsForm({ ...settingsForm, qrisString: e.target.value });
                        setQrisValidation({ checked: false, valid: false, message: '' });
                        setQrCodePreview('');
                      }}
                      placeholder="Mulai dengan 000201..."
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-primary-500 text-xs font-mono min-h-[80px] outline-none"
                    />
                    {settingsForm.qrisString && (
                      <div className="flex gap-2">
                        <button
                          type="button"
                          onClick={handleValidateQris}
                          className="px-3 py-1 bg-primary-100 text-primary-700 hover:bg-primary-200 font-bold text-xs rounded-lg transition-all"
                        >
                          Validasi String
                        </button>
                      </div>
                    ...
                    {qrisValidation.checked && (
                      <div
                        className={`p-2 rounded-lg text-xs font-semibold border ${qrisValidation.valid ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'}`}
                      >
                        {qrisValidation.message}
                      </div>
                    )}

                    {qrCodePreview && (
                      <div className="flex flex-col items-center justify-center p-3 border border-gray-200 rounded-lg bg-white mt-1">
                        <span className="text-[10px] font-bold text-gray-400 mb-2 uppercase">Preview QRIS Anda</span>
                        <img
                          src={qrCodePreview}
                          alt="QR Code Preview"
                          className="w-32 h-32 border object-contain p-1 bg-white"
                        />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="block text-xs font-bold text-gray-600">Gambar QRIS (Max 2 MB)</label>
                    <input
                      type="file"
                      ref={qrisInputRef}
                      accept="image/*"
                    ...
                    {settings.qrisImage && (
                      <div className="flex items-center gap-4 p-2.5 border border-gray-200 rounded-xl bg-white mt-1">
                        <img
                        ...
                        />
                        <div>
                          <span className="block text-xs font-bold text-gray-800">QRIS Statis Tersimpan</span>
                          <button
                            type="button"
            ...
            <form onSubmit={handleProfileSubmit} className="p-5 flex flex-col gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-primary-500 text-sm"
                  required
                />
              </div>
